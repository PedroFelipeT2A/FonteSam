...
if($acao == "ligar_streaming") {
	...
	die("<span class='texto_status_erro'>0x001 - Aten��o! Acesso n�o autorizado, favor entrar em contato com nosso atendimento para maiores informa��es!</span>");
	}
	...
	$total_cameras = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM stmvideo.ip_cameras where codigo_stm = '".$dados_stm["codigo"]."'"));
...
// Fun��o para checar a quantidade de ouvintes online e criar a barra de porcentagem de uso
...
if($acao == "iniciar_playlist") {
	...
	$query_agendamentos = mysqli_query($conexao,"SELECT * FROM playlists_agendamentos where codigo_stm = '".$dados_stm["codigo"]."' ORDER by codigo ASC");
	...
	$total_videos_playlist = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM playlists_videos where codigo_playlist = '".$dados_playlist["codigo"]."'"));
	
	if($total_videos_playlist > 0) {
	
	$query_videos = mysqli_query($conexao,"SELECT * FROM playlists_videos where codigo_playlist = '".$dados_playlist["codigo"]."' ORDER by ordem+0,codigo ASC");
	...
	// Carrega a playlist que ser� iniciada agora
	
	$playlist = "001_".formatar_nome_playlist($dados_playlist_selecionada["nome"]);
	
	$total_videos_playlist = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM playlists_videos where codigo_playlist = '".$dados_playlist_selecionada["codigo"]."'"));
			...
			if($dados_stm["aplicacao"] == 'live' || $dados_stm["aplicacao"] == 'tvstation') {
				echo '<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7;">
				  <tr>
					...
					<td width="75%" align="left" class="texto_padrao_pequeno">rtmp://'.dominio_servidor($conexao,$dados_servidor["nome"]).':1935/'.$dados_stm["login"].'</td>
				  </tr>
				  <tr>
...
if($acao == "gravar_transmissao") {
	...
	// Prote��o contra sess�o expirada
	if(empty($_SESSION["login_logado"])) {
	
	echo "<font color=\"#FF0000\" size=\"4\"><strong>Sess�o Expirada!</strong></font><br><font color=\"#FF0000\" size=\"2\"><strong>Fa�a login novamente.</strong></font>";
	exit();	
	}
	...
	if($dados_servidor["status"] == "off") {
	echo "<font color=\"#999999\" size=\"5\"><strong>". "teste-" . $lang['lang_info_status_manutencao']."</strong></font>";
	exit();
	}
	...
	// Fix permiss�o dos videos gravados
	// Conex�o SSH
	$ssh = new SSH();
	$ssh->conectar($dados_servidor["ip"],$dados_servidor["porta_ssh"]);
	$ssh->autenticar("root",code_decode($dados_servidor["senha"],"D"));
