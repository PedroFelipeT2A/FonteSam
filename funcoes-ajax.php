...
if($acao == "ligar_streaming") {
	...
	$total_cameras = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM ip_cameras where codigo_stm = '".$dados_stm["codigo"]."'"));
...
if($acao == "desligar_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".$lang['lang_acao_desligar_stm_resultado_alerta']."");
...
if($acao == "reiniciar_streaming") {
	...
	if($dados_stm["srt_status"] == 'sim') { 
		@file_get_contents("https://".$_SERVER['HTTP_HOST']."/funcoes-ajax/reiniciar_srt/".query_string('2')."");
	}
...
if($acao == "status_streaming") {
	...
	if($status_streaming["status_transmissao"] == "aovivo" && $dados_stm["relay_status"] == "sim") {
	echo "<font color=\"#009900\" size=\"5\"><strong>".$lang['lang_info_status_transmitindo']."</strong></font><br><font color=\"#009900\" size=\"2\"><strong>Relay</strong></font>";
	exit();
	}
...
if($acao == "gravar_transmissao") {
	...
	// Iniciar gravação	
	$arquivo = "rec_".date("Y-m-d_H-i-s");
	
	$resultado = gravar_transmissao($dados_servidor["ip"],$dados_servidor["senha"],$dados_stm["login"],$arquivo,'iniciar');
	
	if(preg_match('/startRecording/i',$resultado)) {
	
	mysqli_query($conexao,"Update streamings set status_gravando = 'sim', gravador_arquivo = '".$arquivo.".mp4', gravador_data_inicio = NOW() where codigo = '".$dados_stm["codigo"]."'");
	
	echo "iniciado|record/".$arquivo."_*.mp4";
	...
	$ssh->executar("/bin/chown -v streaming.streaming /home/streaming/".$dados_stm["login"]."/*");
...
if($acao == "sincronizar") {
	...
	if($dados_stm["autenticar_live"] == "nao" && ($dados_stm["aplicacao"] == "tvstation" || $dados_stm["aplicacao"] == "live")) {
	$autenticar = "-sem-login";
	}

	if($dados_stm["protecao"] == 'sim') {
	$protecao = "-protecao";
	}

	$aplicacao_xml = ($dados_stm["srt_status"] == 'sim') ? "tvstation" : $aplicacao_xml;
	$autenticar = ($dados_stm["srt_status"] == 'sim') ? "" : $autenticar;
	$protecao = ($dados_stm["srt_status"] == 'sim') ? "" : $protecao;
	...
	$resultado = $ssh->executar("/usr/local/WowzaStreamingEngine/sincronizar ".$dados_stm["login"]." '".$dados_stm["senha_transmissao"]."' ".$dados_stm["bitrate"]." ".$dados_stm["espectadores"]." ".$aplicacao_xml.$autenticar.$protecao."");
	...
	$ssh->executar("perl -i -p -e \"s/<Value>live<\/Value>/<Value>srt.stream<\/Value>/\" /usr/local/WowzaStreamingEngine/conf/".$dados_stm["login"]."/Application.xml;echo OK");
	...
	$ssh->executar("/usr/bin/java -cp /usr/local/WowzaStreamingEngine JMXCommandLine -jmx service:jmx:rmi://localhost:8084/jndi/rmi://localhost:8085/jmxrmi -user admin -pass admin shutdownAppInstance ".$dados_stm["login"]."");

	$ssh->executar("/usr/bin/java -cp /usr/local/WowzaStreamingEngine JMXCommandLine -jmx service:jmx:rmi://localhost:8084/jndi/rmi://localhost:8085/jmxrmi -user admin -pass admin startAppInstance  ".$dados_stm["login"]."");
	...
	$query_ip_cameras = mysqli_query($conexao,"SELECT * FROM ip_cameras WHERE codigo_stm = '".$dados_stm["codigo"]."' ORDER by codigo ASC");
        ...
        <td height="30" align="center" class="texto_padrao_destaque" style="padding-left:5px;">
          <select name="players" class="input" id="players" style="width:98%;" onchange="window.open(this.value,\'conteudo\');">
            ...
            echo '
                    ...
                    <option value="/gerenciar-player-m3u8">Player Próprio / Link M3U8</option></select>
         </td>
      </tr>
    </table>
  </div>
 ...
 <div class="texto_medio" id="quadro-conteudo">
    <table width="575" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7; border:#CCCCCC 1px solid;">
      <tr>
        <td height="30" align="center" class="texto_padrao_destaque" style="padding-left:5px;">
          <select name="players" class="input" id="players" style="width:98%;" onchange="window.open(this.value,\'conteudo\');">
		    <option value="/gerenciar-player-webrtc">'.$lang['lang_info_players_player_selecione'].'</option>
            <option value="/gerenciar-player-webrtc">'.$lang['lang_info_players_player_flash_html5'].'</option>
            </select>
         </td>
      </tr>
    </table>
...
if($acao == "remover_ip_camera") {
	...
	$dados_stm = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM streamings where codigo = '".$dados_camera["codigo_stm"]."'"));
	$dados_servidor = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM servidores where codigo = '".$dados_stm["codigo_servidor"]."'"));
	...
	$resultado = $ssh->executar("rm -fv ".$dados_servidor["path_home"]."/streaming/".$dados_stm["login"]."/".$dados_camera["stream"]."");
	...
	echo "<span class='texto_status_erro'>".$lang['lang_info_gerenciador_ip_cameras_remover_resultado_erro']." ".mysqli_error($conexao)."</span>";
...
// Função para iniciar transmissao de uma camera
if($acao == "iniciar_ip_camera") {
	...
	// Inicia o streaming da camera
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, "http://".$dados_servidor["ip"].":555/streammanager/streamAction?action=startStream&vhostName=_defaultVHost_&appName=".$dados_stm["login"]."%2F_definst_&streamName=".$dados_camera["stream"]."&groupId=&mediaCasterType=rtp");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_TIMEOUT, 5);
	curl_setopt($ch, CURLOPT_USERPWD, "admin:".code_decode($dados_servidor["senha"],"D").""); 
	curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_DIGEST); 
	...
	echo "<span class='texto_status_sucesso'>Transmissão da camera ".$dados_camera["nome"]." iniciada.</span><br /><br/><a href='javascript:void(0);' onClick='document.getElementById(\"log-sistema-fundo\").style.display = \"none\";document.getElementById(\"log-sistema\").style.display = \"none\";' class='texto_status_atualizar'>[".$lang['lang_botao_titulo_fechar']."]</a>";
...
if($acao == "play_video") {
	...
	echo '<iframe src="https://'.$servidor.':1443/play.php?login='.$dados_stm["login"].'&video='.$video.'" frameborder="0" width="320" height="240" scrolling="no"></iframe>';
...
if($acao == "criar_playlist") {
	...
	mysqli_query($conexao,"INSERT INTO playlists (codigo_stm,nome,data) VALUES ('".$dados_stm["codigo"]."','".$playlist."',NOW())");
	...
	echo $lang['lang_acao_gerenciador_playlists_resultado_erro']." ".mysqli_error($conexao)."";
 ...
 <div class="texto_medio" id="quadro-conteudo">
   <table width="575" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7; border:#CCCCCC 1px solid;">
      <tr>
		...
		if($total_videos > 0) {
		echo '<option value="'.code_decode($dados_playlist["codigo"],"E").'">'.$dados_playlist["nome"].' ('.sprintf('%02d:%02d:%02d', ($duracao["total"]/3600),($duracao["total"]/60%60), $duracao["total"]%60).')</option>';
		} else {
...
if($acao == "remover_agendamento_relay") {
	...
	$dados_agendamento_relay = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM relay_agendamentos where codigo = '".$codigo."'"));
	$dados_stm = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM streamings where login = '".$_SESSION["login_logado"]."'"));
	$dados_servidor = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM servidores where codigo = '".$dados_stm["codigo_servidor"]."'"));
	
	mysqli_query($conexao,"Delete From relay_agendamentos where codigo = '".$dados_agendamento_relay["codigo"]."'");
	
	// Remove o arquivo de stream do servidor
	// Conexão FTP
	$ftp = new FTP();
	$ftp->conectar($dados_servidor["ip"]);
	$ftp->autenticar($dados_stm["login"],$dados_stm["senha"]);
	$ftp->remover_arquivo("relay-".md5($dados_agendamento_relay["rtmp"]).".stream");
	
	// Para a transmissao do arquivo de stream do relay no wowza
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, "http://".$dados_servidor["ip"].":555/streammanager/streamAction?action=deleteStream&vhostName=_defaultVHost_&appName=".$dados_stm["login"]."%2F_definst_&streamName=relay-".md5($dados_agendamento_relay["rtmp"]).".stream&groupId=&mediaCasterType=");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_TIMEOUT, 5);
	curl_setopt($ch, CURLOPT_USERPWD, "admin:".code_decode($dados_servidor["senha"],"D").""); 
	curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_DIGEST); 
	...
	// Recarrega as playlists no servidor
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, "http://".$dados_servidor["ip"].":555/schedules?appName=".$dados_stm["login"]."&action=reloadSchedule");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_TIMEOUT, 5);
	curl_setopt($ch, CURLOPT_USERPWD, "admin:".code_decode($dados_servidor["senha"],"D").""); 
	curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_DIGEST); 
	...
	echo "<span class='texto_status_erro'>".$lang['lang_info_remover_agendamento_relay_resultado_erro']." ".mysqli_error($conexao)."</span>";
...
if($acao == "remover_comerciais_playlist") {
	...
	$dados_stm = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM streamings where codigo = '".$dados_playlist["codigo_stm"]."'"));
	$dados_servidor = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM servidores where codigo = '".$dados_stm["codigo_servidor"]."'"));
	...
	echo "<span class='texto_status_erro'>".$lang['lang_info_gerenciador_playlists_remover_comerciais_resultado_erro']." ".mysqli_error($conexao)."</span>";
...
if($acao == "duplicar_playlist") {
	...
	$dados_stm = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM streamings where codigo = '".$dados_playlist_atual["codigo_stm"]."'"));
	$dados_servidor = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM servidores where codigo = '".$dados_stm["codigo_servidor"]."'"));

	mysqli_query($conexao,"INSERT INTO playlists (codigo_stm,nome,data) VALUES ('".$dados_stm["codigo"]."','".$playlist_nova."',NOW())");
	$codigo_playlist = mysqli_insert_id($conexao);
	...
	echo "<span class='texto_status_erro'>".$lang['lang_info_gerenciador_playlists_duplicar_resultado_erro']." ".mysqli_error($conexao)."</span>";	
...
if($acao == "remover_app_android") {
	...
	@file_get_contents("https://app.srvstm.com/remover-app.php?app=".$dados_app["zip"]."");
...
if($acao == "reiniciar_srt") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".$lang['lang_acao_reiniciar_stm_resultado_ok']."(SRT)");
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".$lang['lang_acao_reiniciar_stm_resultado_erro']."(SRT)");
