...
$sql = mysqli_query($conexao,"SELECT * FROM servidores where status = 'on' ORDER by codigo ASC");
...
mysqli_query($conexao,"UPDATE `servidores` SET `load` = '".$load."', `trafego` = '".str_replace("iB","b",$trafego)."', `trafego_out` = '".str_replace("it/s","/s",$trafego_out)."' WHERE `servidores`.`codigo` = '".$dados_servidor["codigo"]."'") or die(mysqli_error($conexao));
