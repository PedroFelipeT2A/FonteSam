...
$dados_playlist_selecionada = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM playlists where codigo = '".code_decode(query_string('1'),"D")."'"));
...
$duracao = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(duracao_segundos) as total FROM playlists_videos where codigo_playlist = '".$dados_playlist_selecionada["codigo"]."'"));
...
$dados_playlist = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM playlists where codigo = '".$_POST["playlist"]."'"));
...
$total_videos_atuais_playlist = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM playlists_videos where codigo_playlist = '".$dados_playlist_selecionada["codigo"]."'"));
...
mysqli_query($conexao,"INSERT INTO playlists_videos (codigo_playlist,path_video,video,width,height,bitrate,duracao,duracao_segundos,tipo,ordem) VALUES ('".$dados_playlist["codigo"]."','".addslashes($path)."','".addslashes($video)."','".$width."','".$height."','".$bitrate."','".$duracao."','".$duracao_segundos."','video','".$contagem."')") or die("Ooops! Ocorreu um erro no mysql: ".mysqli_error($conexao));
...
$query_playlists = mysqli_query($conexao,"SELECT * FROM playlists where codigo_stm = '".$dados_stm["codigo"]."' ORDER by nome ASC");
while ($dados_playlist_gerenciar = mysqli_fetch_array($query_playlists)) {

$total_videos = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM playlists_videos where codigo_playlist = '".$dados_playlist_gerenciar["codigo"]."'"));
...
?>
        ...
        <td width="217" align="center" scope="col"><table width="200" border="0" align="right" cellpadding="0" cellspacing="0" style="border: #CCCCCC 1px solid;">
          ...
          <tr>
            <td height="40" align="center" class="texto_padrao_titulo" scope="col"><?php echo sprintf('%02d:%02d:%02d', ($duracao["total"]/3600),($duracao["total"]/60%60), $duracao["total"]%60); ?></td>
          </tr>
        </table></td>
      </tr>
    </table>
