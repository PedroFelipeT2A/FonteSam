...
?>
  <table width="870" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-bottom:5px;">
    <tr>
      ...
      <td width="300" align="left" scope="col"><a href="/admin/admin-cadastrar-aviso" class="texto_padrao_destaque">Cadastrar Aviso</a></td>
  ...
  <table width="870" border="0" align="center" cellpadding="0" cellspacing="0" style=" border-top:#D5D5D5 1px solid; border-left:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;" id="tab" class="sortable">
    <tr style="background:url(img/img-fundo-titulo-tabela.png) repeat-x; cursor:pointer">
      <td width="300" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Titulo</td>
      ...
      <td width="100" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Área</td>
      <td width="80" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Exibir</td>
...
$sql = mysqli_query($conexao,"SELECT * FROM avisos");
$lpp = 100; // total de registros por p&aacute;gina
$total = mysqli_num_rows($sql);
$paginas = ceil($total / $lpp); 
if(!is_numeric($pagina_atual)) { $pagina_atual = 0; }
$inicio = $pagina_atual * $lpp;
$sql = mysqli_query($conexao,"SELECT *, DATE_FORMAT(data,'%d/%m/%Y') AS data FROM avisos ORDER by codigo ASC LIMIT $inicio, $lpp");

while ($dados_aviso = mysqli_fetch_array($sql)) {

$total_vizualizacoes = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM avisos_desativados where codigo_aviso = '".$dados_aviso["codigo"]."'"));

if($dados_aviso["area"] == "streaming") {
$area = "Streaming";
} else {
$area = "Revenda";
}
...
$aviso_code = code_decode($dados_aviso["codigo"],"E");

echo "<tr style='background-color:#FFFFFF;'>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;".$dados_aviso["titulo"]."</td>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;".$dados_aviso["data"]."</td>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;".$area."</td>
...
<select style='width:100%' id='".$aviso_code."' onchange='executar_acao_diversa(this.id,this.value);'>
  <option value='' selected='selected'>Escolha uma ação</option>
  <option value='alterar-status-aviso'>Status Sim/Não</option>
  <option value='remover-aviso'>Remover</option>
  </optgroup>
</select>
</td>
</tr>";
...
$total_registros = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM avisos"));

if($total_registros == 0) {
echo "<span class=\"texto_padrao_destaque\">Nenhum aviso encontrado.</span>";
} else {
