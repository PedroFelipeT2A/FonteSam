...
if($acao == "ligar_streaming") {
	...
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_ligar_stm_resultado_ok."");
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_ligar_stm_resultado_erro."");
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_ligar_stm_resultado_alerta."");
...
if($acao == "desligar_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_desligar_stm_resultado_ok."");
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_desligar_stm_resultado_erro."");
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_desligar_stm_resultado_alerta."");
...
if($acao == "reiniciar_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_reiniciar_stm_resultado_ok."");
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".lang_acao_reiniciar_stm_resultado_erro."");
...
if($acao == "status_streaming") {
	...
	if($dados_stm["srt_status"] == 'sim') {
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url_source_srt);
...
if($acao == "bloquear_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] ".lang_acao_bloquear_stm_resultado_ok." pela revenda.");
...
if($acao == "desbloquear_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] ".lang_acao_desbloquear_stm_resultado_ok." pela revenda.");
...
if($acao == "remover_streaming") {
	...
	logar_acao($conexao,"[".$dados_stm["login"]."] Remoção do streaming pela revenda.");
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] ".lang_acao_remover_stm_resultado_ok." pela revenda.");
	
	} else {
	
	echo "<span class='texto_status_erro'>".lang_acao_remover_stm_resultado_erro."<br>Log: ".mysqli_error($conexao)."</span>";
	
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] ".lang_acao_remover_stm_resultado_erro." Log: ".mysqli_error($conexao)."");
...
if($acao == "acessar_painel_streaming_revenda") {
	...
	$dados_stm = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM streamings where login = '".code_decode(query_string('3'),"D")."'"));
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] Acesso da revenda ao painel de streaming executado com sucesso.");
...
if($acao == "sincronizar") {
	...
	$query_ip_cameras = mysqli_query($conexao,"SELECT * FROM stmvideo.ip_cameras WHERE codigo_stm = '".$dados_stm["codigo"]."' ORDER by codigo ASC");
	while ($dados_ip_camera = mysqli_fetch_array($query_ip_cameras)) {
...
<div id="quadro-topo"><strong>'.lang_info_pagina_resolver_problemas_tab_titulo_facebook.'</strong></div>
 <div class="texto_medio" id="quadro-conteudo">
   <table width="575" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7; border:#CCCCCC 1px solid;">
      <tr>
        <td height="30" align="left" class="texto_padrao"><br />'.sprintf(lang_acao_pagina_resolver_problemas_player_facebook,"player.".$dados_config["dominio_padrao"],$dados_stm["login"],"player.".$dados_config["dominio_padrao"],$dados_stm["login"]).'<br /></td>
      </tr>
    </table>
  </div>
</div>';
...
if($acao == "desativar_exibicao_aviso") {
	...
	mysqli_query($conexao,"INSERT INTO avisos_desativados (codigo_aviso,codigo_usuario,area,data) VALUES ('".$codigo_aviso."','".$codigo_usuario."','".$area."',NOW())");
...
if($acao == "desbloquear_ip_login") {
	...
	mysqli_query($conexao,"Delete From bloqueios_login where codigo = '".$codigo."'");
...
if($acao == "bloquear_subrevenda") {
	...
	$dados_subrevenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas where codigo = '".$codigo."'"));
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_subrevenda["nome"]." - ".$dados_subrevenda["email"]."] Sub Revenda bloqueada com sucesso");
...
if($acao == "desbloquear_subrevenda") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_subrevenda["nome"]." - ".$dados_subrevenda["email"]."] Sub Revenda desbloqueada com sucesso");
...
if($acao == "remover_subrevenda") {
	...
	$checar_subrevenda = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".$codigo."')"));
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] Streaming removido com sucesso na remoção da sub revenda ".$dados_subrevenda["id"]." - ".$dados_subrevenda["email"]."");
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_subrevenda["nome"]." - ".$dados_subrevenda["email"]."] Sub Revenda removida com sucesso.");
...
if($acao == "mover_streaming_subrevenda_revenda") {
	...
	$verifica_subrevenda = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".$dados_stm["codigo_cliente"]."') AND tipo = '2'"));
	
	// Verifica se o streaming é de uma sub revenda que pertence a revenda logada
	if($verifica_subrevenda > 0) {
	
	mysqli_query($conexao,"Update streamings set codigo_cliente = '".$dados_revenda["codigo"]."' where codigo = '".$dados_stm["codigo"]."'");
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] Movido para revenda principal ".$dados_revenda["id"]."");
...
if($acao == "mover_streaming_subrevenda_subrevenda") {
	...
	$verifica_subrevenda_atual = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".$dados_stm["codigo_cliente"]."') AND tipo = '2'"));
	
	$verifica_subrevenda_nova = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND id = '".$id."') AND tipo = '2'"));
	
	// Verifica se o streaming é de uma sub revenda que pertence a revenda logada
	if($verifica_subrevenda_atual > 0 && $verifica_subrevenda_nova > 0) {
	
	$dados_subrevenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE id = '".$id."'"));

	mysqli_query($conexao,"Update streamings set codigo_cliente = '".$dados_subrevenda["codigo"]."' where codigo = '".$dados_stm["codigo"]."'");
	
	echo "<span class='texto_status_sucesso'>".lang_info_subrevenda_streamings_mover_streaming_resultado_ok."</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[".lang_botao_titulo_atualizar."]</a>";
	
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] Movido para sub revenda ".$dados_subrevenda["id"]."");
...
if($acao == "buscar_subrevenda") {
	...
	$verifica_subrevenda = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND id = '".query_string('3')."') AND tipo = '2'"));
	
	if($verifica_subrevenda > 0) {
	
	$dados_subrevenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND id = '".query_string('3')."') AND tipo = '2'"));
...
if($acao == "remover_plano") {
	...
	mysqli_query($conexao,"Delete From revendas_planos where codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".$plano."'");
...
if($acao == "remover_app_android") {
	...
	echo "<span class='texto_status_erro'>Não foi possível remover a requisição do streaming <strong>".$dados_stm["login"]."</strong>.</strong><br>Log: ".mysqli_error($conexao)."</span>";
