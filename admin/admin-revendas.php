...
$query = "SELECT * FROM revendas where status = '".query_string('3')."'";
$pagina_atual = query_string('4');
...
$sql = mysqli_query($conexao,"".$query." ORDER by nome ASC LIMIT $inicio, $lpp");
...
$link_financeiro = "<a href='https://financeiro.painelcast.com/index.php?pagina=Busca&cat=dominio&chave=revenda-stm-video-".$dados_revenda["id"]."' target='_blank'><img src='https://financeiro.painelcast.com/img/icones/busca.png' alt='Buscar no Financeiro' width='10' height='10' border='0' align='absmiddle' /></a>";
...
$sql_subrevenda = mysqli_query($conexao,"SELECT * FROM revendas where codigo_revenda = '".$dados_revenda["codigo"]."' AND tipo = '2' ORDER by id ASC");
...
// Verifique se tem subrevenda e lista
$total_subrevendas_sub = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE codigo_revenda = '".$dados_subrevenda["codigo"]."' AND tipo = '3'"));

if($total_subrevendas_sub > 0) {

$sql_subrevenda_sub = mysqli_query($conexao,"SELECT * FROM revendas where codigo_revenda = '".$dados_subrevenda["codigo"]."' AND tipo = '3' ORDER by id ASC");
