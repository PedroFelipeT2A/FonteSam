...
$sql = mysqli_query($conexao,"SELECT * FROM tutoriais");
$lpp = 100; // total de registros por p&aacute;gina
$total = mysqli_num_rows($sql);
$paginas = ceil($total / $lpp); 
if(!is_numeric($pagina_atual)) { $pagina_atual = 0; }
$inicio = $pagina_atual * $lpp;
$sql = mysqli_query($conexao,"SELECT *, DATE_FORMAT(data,'%d/%m/%Y') AS data FROM tutoriais ORDER by codigo ASC LIMIT $inicio, $lpp");
...
$total_registros = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM tutoriais"));
