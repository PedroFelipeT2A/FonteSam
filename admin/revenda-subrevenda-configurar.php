...
$dados_subrevenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".code_decode(query_string('2'),"D")."')"));
  ...
  <form method="post" action="/admin/revenda-subrevenda-configura" style="padding:0px; margin:0px">
    <table width="640" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7; border:#CCCCCC 1px solid;">
      ...
      <tr>
		...
		<?php
        $query_plano = mysqli_query($conexao,"SELECT * FROM revendas_planos where codigo_revenda = '".$dados_revenda["codigo"]."' AND tipo = 'subrevenda' ORDER by nome");
        while ($dados_plano = mysqli_fetch_array($query_plano)) {
      ...
      <tr>
        ...
        <td align="left"><input name="streamings" type="number" class="input" id="streamings" style="width:250px;" value="<?php echo $dados_subrevenda["streamings"]; ?>" />&nbsp;
      ...
      <tr>
        <td height="30" align="left" class="texto_padrao_destaque" style="padding-left:5px;"><?php echo lang_info_pagina_configurar_subrevenda_limite_espectadores; ?></td>
        <td align="left"><input name="espectadores" type="number" class="input" id="espectadores" style="width:250px;" value="<?php echo $dados_subrevenda["espectadores"]; ?>" />&nbsp;
        <?php if($dados_revenda["espectadores"] == '999999') { ?>
        <input type="checkbox" id="espectadores_ilimitados" onclick="configurar_ilimitado('espectadores_ilimitados','espectadores');" style="vertical-align:middle" />&nbsp;<span class="texto_ilimitado"><?php echo lang_info_pagina_configurar_subrevenda_ilimitado; ?></span>
        <?php } ?>        </td>
      </tr>
      <tr>
        ...
        <td align="left" class="texto_padrao_pequeno"><input name="bitrate" type="number" class="input" id="bitrate" style="width:250px;" value="<?php echo $dados_subrevenda["bitrate"]; ?>" /></td>
      </tr>
      <tr>
        ...
        <td align="left" class="texto_padrao_pequeno"><input name="espaco" type="number" class="input" id="espaco" style="width:250px;" value="<?php echo $dados_subrevenda["espaco"]; ?>" />&nbsp;Megabytes</td>
      </tr>
      <tr>
        ...
        <select name="idioma_painel" id="idioma_painel" style="width:255px;">
          <option value="pt-br"<?php if($dados_subrevenda["idioma_painel"] == "pt-br") { echo ' selected="selected"'; } ?>>Português(Brasil)</option>
          <option value="es"<?php if($dados_subrevenda["idioma_painel"] == "es") { echo ' selected="selected"'; } ?>>Español</option>
...
<table width="340" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7; border:#CCCCCC 1px solid; margin-top:10px; margin-bottom:5px;">
        ...
        <tr>
          <td height="25" align="left" class="texto_padrao_destaque" style="padding-left:5px;"><?php echo lang_info_pagina_configurar_subrevenda_estatisticas_espectadores; ?></td>
