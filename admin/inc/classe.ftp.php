...
class FTP {
  ...
  /*
     if(ftp_size($this->conexao, $pasta) == '-1' && !in_array($pasta, array('.', '..'))){
       return true;
     } else {
       return false;
     }
  ...
  function remover_pasta($pasta) {
     ...
     if (!empty($this->arquivos)) {
       ...
       foreach($this->arquivos as $arquivo) {
        ...
        if(!$this->verificar_pasta($arquivo)) {
          if(strstr($arquivo, '/')){
            ftp_delete($this->conexao,$arquivo);
          } else {
