...
@mysqli_query($conexao,"ALTER TABLE `servidores` ADD `path_home` VARCHAR(255) NOT NULL DEFAULT '/home';");
...
<form style="padding:0; margin:0" onsubmit="buscar_servidor(document.getElementById('chave').value);return false;">
  <table width="1100" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-bottom:5px;">
    <tr>
      ...
      <td width="90" align="left" scope="col"><a href="/admin/admin-cadastrar-servidor" class="texto_padrao_destaque">Cadastrar</a></td>
      <td width="30" align="center" class="texto_padrao_destaque" scope="col"><img src="/admin/img/icones/img-icone-configuracoes.png" alt="Instalar Servidor" width="16" height="16" /></td>
      <td width="145" align="left" scope="col"><a href="/admin/admin-instalar-servidor" target="_blank" class="texto_padrao_destaque">Instalar Novo Servidor</a></td>
      <td width="30" align="center" class="texto_padrao_destaque" scope="col"><img src="/admin/img/icones/img-icone-configuracoes.png" alt="Instalar Servidor" width="16" height="16" /></td>
      <td width="115" align="left" scope="col"><a href="/admin/admin-migrar-servidor" target="_blank" class="texto_padrao_destaque">Migrar Servidor</a></td>
      ...
      <td width="140" align="left" scope="col"><a href="javascript:ligar_streamings_geral();" class="texto_padrao_destaque">Ligar Streamings</a></td>
      <td width="210" align="left" class="texto_padrao_destaque" scope="col"><input style="vertical-align:middle;" type="radio" name="filtro" onclick="window.location = '/admin/admin-servidores';" <?php if(!query_string('2')) { echo 'checked="checked"'; } ?> />
        &nbsp;Todos
        ...
        &nbsp;Inativos </td>
      <td width="290" align="right" class="texto_padrao_destaque" scope="col">
      Buscar por
        <input name="chave" type="text" id="chave" style="width:120px" />
        <input type="button" class="botao_padrao" value="Buscar" onclick="buscar_servidor(document.getElementById('chave').value);" /></td>
    </tr>
  </table>
</form>
  <table width="1100" border="0" align="center" cellpadding="0" cellspacing="0" style=" border-top:#D5D5D5 1px solid; border-left:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;" id="tab" class="sortable">
    <tr style="background:url(/img/img-fundo-titulo-tabela.png) repeat-x; cursor:pointer">
      <td width="120" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Nome</td>
      ...
      <td width="150" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Domínio</td>
      <td width="75" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Porta</td>
      ...
      <td width="75" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Load</td>
      <td width="120" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Tráfego Rede</td>
      <td width="120" height="23" align="left" class="texto_padrao_destaque" style="border-bottom:#D5D5D5 1px solid; border-right:#D5D5D5 1px solid;">&nbsp;Tráfego do Mês</td>
...
$query = "SELECT * FROM servidores where status = '".query_string('3')."'";
$pagina_atual = query_string('4');
...
if(!is_numeric($pagina_atual)) { $pagina_atual = 0; }
$inicio = $pagina_atual * $lpp;
$sql = mysqli_query($conexao,"".$query." ORDER by ordem ASC LIMIT $inicio, $lpp");
...
$servidor_principal = "<span class='texto_padrao_pequeno'>(".$dados_servidor["nome_principal"].")</span>";
}

if($dados_servidor["instalacao_status"] == "0") {

echo "<tr style='background-color:".$cor_alerta_linha.";' title='Código: ".$dados_servidor["codigo"]." | Nome: ".$dados_servidor["nome"]." | IP: ".$dados_servidor["ip"]." | Servidor Principal: ".$dados_servidor["nome_principal"]."'>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;".$dados_servidor["nome"]." ".$servidor_principal."</td>
...
<select style='width:100%' id='".$servidor_code."' onchange='executar_acao_servidor(this.id,this.value);'>
  ...
  <option value='listar-conexoes'>Listar Conexões</option>
  </optgroup>
  ...
  <option value='manutencao-servidor'>Efetuar Manuten&ccedil;&atilde;o</option>
...
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;".$dados_servidor["porta_ssh"]."</td>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;".$total_stm." / ".$dados_servidor["limite_streamings"]."</td>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;</td>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;</td>
<td height='25' align='left' scope='col' class='texto_padrao'>&nbsp;</td>
<td height='25' align='center' scope='col' class='texto_padrao'><a href='/admin/admin-instalar-servidor/".$servidor_code."' target='_blank'>[Verificar Instalação]</a></td>
</tr>";
}

unset($alerta_load);
unset($alerta_trafego_out);
unset($servidor_principal);
}
?>
  </table>
  <table width="1100" border="0" align="center" cellpadding="0" cellspacing="0" style=" border:#D5D5D5 1px solid;">
    <tr>
      <td height="20" align="center"><?php
$total_registros = mysqli_num_rows(mysqli_query($conexao,"".$query.""));
