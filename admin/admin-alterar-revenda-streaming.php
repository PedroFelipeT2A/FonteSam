...
$dados_revenda_atual = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE codigo = '".code_decode(query_string('2'),"D")."'"));
?>
...
<?php
$query_revenda = mysqli_query($conexao,"SELECT * FROM revendas ORDER by nome ASC");
while ($dados_revenda = mysqli_fetch_array($query_revenda)) {
...
<?php
$sql_streamings = mysqli_query($conexao,"SELECT * FROM streamings where codigo_cliente = '".$dados_revenda_atual["codigo"]."' ORDER by login ASC");
while ($dados_stm = mysqli_fetch_array($sql_streamings)) {
?>
      <tr>
