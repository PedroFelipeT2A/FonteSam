  ...
  <form method="post" action="/admin/admin-configura-servidor" style="padding:0px; margin:0px">
    <table width="500" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7; border:#CCCCCC 1px solid;">
      ...
      <tr>
        ...
        <td align="left"><input name="senha" type="password" class="input" id="senha" style="width:250px;" value="<?php echo code_decode($dados_servidor["senha"],"D"); ?>" /></td>
      </tr>
      <tr>
      ...
      <tr>
        <td height="30" align="left" style="padding-left:5px;" class="texto_padrao_destaque">Caminho Home</td>
        <td align="left"><input name="path_home" type="text" class="input" id="path_home" style="width:250px;" value="<?php echo $dados_servidor["path_home"]; ?>" /></td>
      </tr>
      <tr>
      ...
      <tr>
        <td height="30" align="left" style="padding-left:5px;" class="texto_padrao_destaque">Servidor Principal</td>
        <td align="left" class="texto_padrao_pequeno"><select name="nome_principal" class="input" id="nome_principal" style="width:255px;">
        <option value=""<?php if(empty($dados_servidor["nome_principal"])) { echo ' selected="selected"'; } ?>>Servidor Único</option>
            <?php

$query = mysqli_query($conexao,"SELECT * FROM servidores ORDER by ordem ASC");
while ($dados_servidor_principal = mysqli_fetch_array($query)) {

$total_streamings = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM streamings where codigo_servidor = '".$dados_servidor_principal["codigo"]."'"));

if(strtolower($dados_servidor_principal["nome"]) == strtolower($dados_servidor["nome_principal"])) {
echo '<option value="' . strtolower($dados_servidor_principal["nome"]) . '" selected="selected">' . $dados_servidor_principal["nome"] . ' - ' . $dados_servidor_principal["ip"] . '</option>';
} else {
echo '<option value="' . strtolower($dados_servidor_principal["nome"]) . '">' . $dados_servidor_principal["nome"] . ' - ' . $dados_servidor_principal["ip"] . '</option>';
}
...
?>
          ...
          <br /> 
        (nome do servidor principal caso este servidor não seja único ou selecione servidor único)</td>
      </tr>
      <tr>
