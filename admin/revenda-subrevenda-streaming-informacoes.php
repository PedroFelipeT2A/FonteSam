...
$dados_subrevenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".$dados_stm["codigo_cliente"]."') AND tipo = '2'"));
$dados_servidor = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM servidores where codigo = '".$dados_stm["codigo_servidor"]."'"));

$rtmp_aacplus = ($dados_stm["aacplus"] == "sim") ? "rtmp://".dominio_servidor($conexao,$dados_servidor["nome"])."/".$dados_stm["login"]."" : "<span class='texto_padrao_vermelho'>".lang_info_pagina_informacoes_subrevenda_streaming_tab_stm_rtmp_info_desativado."</span>";
      ...
      <div class="texto_medio" id="quadro-conteudo">
        <table width="427" border="0" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                <td align="left" class="texto_padrao_pequeno"><a href="javascript:abrir_janela('http://<?php echo dominio_servidor($conexao,$dados_servidor["nome"]); ?>:<?php echo $dados_stm["login"]; ?>',720,500);">http://<?php echo dominio_servidor($conexao,$dados_servidor["nome"]); ?>:<?php echo $dados_stm["login"]; ?></a></td>
              </tr>
              <tr>
...
<?php
carregar_avisos_streaming_revenda($conexao,$dados_stm["login"],$dados_servidor["codigo"]);
?>
</td>
    </tr>
</table>
