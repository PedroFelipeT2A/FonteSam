...
$dados_revenda_atual = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE codigo = '".$_POST["codigo_cliente"]."'"));
$dados_revenda_nova = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE codigo = '".$revenda."'"));

mysqli_query($conexao,"UPDATE streamings set codigo_cliente = '".$dados_revenda_nova["codigo"]."' where login = '".$dados_stm["login"]."'");
