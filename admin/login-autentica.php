...
$valida_revenda = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE email = '".$x_email."' AND senha = PASSWORD('".$x_senha."')"));

if($valida_revenda == 1) {

$dados_revenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE email = '".$x_email."' AND senha = PASSWORD('".$x_senha."')"));

$_SESSION["type_logged_user"] = "cliente";
$_SESSION["code_user_logged"] = $dados_revenda["codigo"];

// Loga o acesso do usuario
mysqli_query($conexao,"INSERT INTO logs (acao,data,ip,log) VALUES ('login',NOW(),'".$_SERVER['REMOTE_ADDR']."','Revenda ".$_POST["email"]." acessou sistema.')");

// Loga ultimo acesso da revenda
@mysqli_query($conexao,"Update revendas set ultimo_acesso_data = NOW(), ultimo_acesso_ip = '".$_SERVER['REMOTE_ADDR']."'  WHERE codigo = '".$dados_revenda["codigo"]."'");
...
$valida_operador = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM administradores WHERE usuario = '".$x_email."' AND senha = PASSWORD('".$x_senha."')"));

if($valida_operador == 1) {

$dados_operador = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM administradores WHERE usuario = '".$x_email."'"));

$_SESSION["type_logged_user"] = "operador";
$_SESSION["code_user_logged"] = $dados_operador["codigo"];

// Loga o acesso do usuario
mysqli_query($conexao,"INSERT INTO logs (acao,data,ip,log) VALUES ('login',NOW(),'".$_SERVER['REMOTE_ADDR']."','Administrador ".$_POST["email"]." acessou sistema.')");
