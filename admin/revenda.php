...
$verifica_stm = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM streamings where login = '".code_decode(query_string('2'),"D")."'"));
...
$verifica_subrevenda = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".code_decode(query_string('3'),"D")."') AND tipo = '".$tipo_subrevenda."'"));
...
$dados_subrevenda_atual = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas WHERE (codigo_revenda = '".$dados_revenda["codigo"]."' AND codigo = '".code_decode(query_string('3'),"D")."') AND tipo = '".$tipo_subrevenda."'"));

// Estatísticas de Uso do Plano
$total_streamings = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM streamings WHERE codigo_cliente = '".$dados_subrevenda_atual["codigo"]."'"));
$espectadores = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(espectadores) as total FROM streamings WHERE codigo_cliente = '".$dados_subrevenda_atual["codigo"]."'"));
$espaco = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(espaco) as total FROM streamings WHERE codigo_cliente = '".$dados_subrevenda_atual["codigo"]."'"));
...
<?php
$i = 0;
$sql = mysqli_query($conexao,"SELECT * FROM streamings where codigo_cliente = '".$dados_revenda["codigo"]."' AND status = '1' ORDER by login ASC");
while ($dados_lista_stm = mysqli_fetch_array($sql)) {
	
	$dados_servidor_lista_stm = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM servidores where codigo = '".$dados_lista_stm["codigo_servidor"]."'"));
	$limite_espectadores = ($dados_lista_stm["espectadores"] == 999999) ? lang_info_ilimitado : $dados_lista_stm["espectadores"];

	echo "<option value='".code_decode($dados_lista_stm["login"],"E")."' style='background-color: ".zebrar($i, "#F5F5F5", "#FFFFFF")."'>".$dados_lista_stm["login"]." - ".dominio_servidor($conexao,$dados_servidor_lista_stm["nome"])." - ".$limite_espectadores." espectadores/".tamanho($dados_lista_stm["espaco"])."/".$dados_lista_stm["bitrate"]."Kbps (".$dados_lista_stm["identificacao"].")</option>";
...
while ($dados_lista_stm = mysqli_fetch_array($sql)) {
	...
	echo "<option value='' disabled='disabled' style='background-color: ".zebrar($i, "#F5F5F5", "#FFFFFF")."'>".$dados_lista_stm["login"]." - ".dominio_servidor($conexao,$dados_servidor_lista_stm["nome"])." - ".$limite_espectadores." espectadores/".tamanho($dados_lista_stm["espaco"])."/".$dados_lista_stm["bitrate"]."Kbps (".$dados_lista_stm["identificacao"].")</option>";
	} else {
	echo "<option value='".code_decode($dados_lista_stm["login"],"E")."' style='background-color: ".zebrar($i, "#F5F5F5", "#FFFFFF")."'>".$dados_lista_stm["login"]." - ".dominio_servidor($conexao,$dados_servidor_lista_stm["nome"])." - ".str_replace("999999","Ilimitados",$dados_lista_stm["espectadores"])." espectadores/".tamanho($dados_lista_stm["espaco"])."/".$dados_lista_stm["bitrate"]."Kbps (".$dados_lista_stm["identificacao"].")</option>";
	}
...
<?php
$i = 0;
$sql = mysqli_query($conexao,"SELECT * FROM revendas where codigo_revenda = '".$dados_revenda["codigo"]."' AND status = '1' AND tipo = '".$tipo_subrevenda."' ORDER by codigo ASC");
while ($dados_subrevendas_lista = mysqli_fetch_array($sql)) {
