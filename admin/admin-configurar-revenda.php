...
$dados_revenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM revendas where codigo = '".$revenda_code."'"));
...
$espectadores_subrevendas = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(espectadores) as total FROM revendas WHERE codigo_revenda = '".$dados_revenda["codigo"]."' AND tipo = '2'"));
$espaco_subrevendas = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(espaco) as total FROM revendas WHERE codigo_revenda = '".$dados_revenda["codigo"]."' AND tipo = '2'"));
  ...
  <form method="post" action="/admin/admin-configura-revenda" style="padding:0px; margin:0px">
    <table width="500" border="0" align="center" cellpadding="0" cellspacing="0" style="background-color:#F4F4F7; border:#CCCCCC 1px solid;">
      ...
      <tr>
        ...
        <input type="radio" name="srt_status" id="srt_status" value="sim" <?php if($dados_revenda["srt_status"] == "sim") { echo 'checked="checked"';} ?> />&nbsp;Sim
        <input type="radio" name="srt_status" id="srt_status" value="nao" <?php if($dados_revenda["srt_status"] == "nao") { echo 'checked="checked"';} ?> />&nbsp;N&atilde;o
        </td>
      </tr>
      <tr>
        <td height="40">
