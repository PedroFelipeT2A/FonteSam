...
if($_POST["subrevenda_email"] != $dados_subrevenda_atual["email"]) {
	$verificacao_email = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM revendas where email = '".$_POST["subrevenda_email"]."'"));
...
$total_streamings_subrevenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(streamings) as total FROM revendas WHERE codigo_revenda = '".$dados_revenda["codigo"]."'"));
...
$espectadores_subrevenda_revenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(espectadores) as total FROM revendas WHERE codigo_revenda = '".$dados_revenda["codigo"]."'"));
...
$espaco_subrevenda_revenda = mysqli_fetch_array(mysqli_query($conexao,"SELECT SUM(espaco) as total FROM revendas WHERE codigo_revenda = '".$dados_revenda["codigo"]."'"));
...
mysqli_query($conexao,"Update revendas set nome = '".$dados_revenda["nome"]."', email = '".$_POST["subrevenda_email"]."', senha = PASSWORD('".$_POST["subrevenda_senha"]."'), streamings = '".$_POST["streamings"]."', espectadores = '".$_POST["espectadores"]."', bitrate = '".$_POST["bitrate"]."', espaco = '".$_POST["espaco"]."', chave_api = '".code_decode($_POST["subrevenda_email"],"E")."', idioma_painel = '".$_POST["idioma_painel"]."', subrevendas = '".$_POST["subrevendas"]."' where codigo = '".$dados_subrevenda_atual["codigo"]."'") or die("Erro ao processar query.<br>Mensagem do servidor: ".mysqli_error($conexao));

} else {

mysqli_query($conexao,"Update revendas set nome = '".$dados_revenda["nome"]."', email = '".$_POST["subrevenda_email"]."', streamings = '".$_POST["streamings"]."', espectadores = '".$_POST["espectadores"]."', bitrate = '".$_POST["bitrate"]."', espaco = '".$_POST["espaco"]."', chave_api = '".code_decode($_POST["subrevenda_email"],"E")."', idioma_painel = '".$_POST["idioma_painel"]."', subrevendas = '".$_POST["subrevendas"]."' where codigo = '".$dados_subrevenda_atual["codigo"]."'") or die("Erro ao processar query.<br>Mensagem do servidor: ".mysqli_error($conexao));

}

// Insere a ação executada no registro de logs.
logar_acao($conexao,"[".$dados_subrevenda_atual["id"]."] Configurações da Sub revenda alteradas com sucesso para revenda ".$dados_revenda["nome"]." - ".$dados_revenda["id"]."");
