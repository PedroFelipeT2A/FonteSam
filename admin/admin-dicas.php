...
<?php
$sql = mysqli_query($conexao,"SELECT * FROM dicas_rapidas");
$lpp = 100; // total de registros por p&aacute;gina
$total = mysqli_num_rows($sql);
$paginas = ceil($total / $lpp); 
if(!is_numeric($pagina_atual)) { $pagina_atual = 0; }
$inicio = $pagina_atual * $lpp;
$sql = mysqli_query($conexao,"SELECT * FROM dicas_rapidas ORDER by codigo ASC LIMIT $inicio, $lpp");

while ($dados_dica_rapida = mysqli_fetch_array($sql)) {

$total_vizualizacoes = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM dicas_rapidas_acessos where codigo_dica = '".$dados_dica_rapida["codigo"]."'"));
...
$total_registros = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM dicas_rapidas"));
