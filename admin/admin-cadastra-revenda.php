...
mysqli_query($conexao,"INSERT INTO revendas (id,nome,email,senha,subrevendas,streamings,espectadores,bitrate,espaco,chave_api,data_cadastro,srt_status) VALUES ('".$_POST["id"]."','".$_POST["nome"]."','".$_POST["email"]."',PASSWORD('".$_POST["senha"]."'),'".$_POST["subrevendas"]."','".$_POST["streamings"]."','".$_POST["espectadores"]."','".$_POST["bitrate"]."','".$_POST["espaco"]."','".code_decode($_POST["email"],"E")."',NOW(),'".$srt_status."')") or die("Erro ao processar query.<br>Mensagem do servidor: ".mysqli_error($conexao));
$codigo_revenda = mysqli_insert_id($conexao);

// Loga a ação executada
mysqli_query($conexao,"INSERT INTO logs (acao,data,ip,log) VALUES ('cadastro_revenda',NOW(),'".$_SERVER['REMOTE_ADDR']."','Cadastrada revenda ".$_POST["nome"]." email ".$_POST["email"]."')");
