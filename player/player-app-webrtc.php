...
$array_lang = array(
	"pt-br" => array("redes_sociais" => "Redes Sociais", "avaliar" => "Avaliar", "politica" => "Pol&iacute;tica & Privacidade", "compartilhar" => "Compartilhar", "pedir_musica" => "Pedir M&uacute;sica", "voltar" => "Voltar", "aviso_bg" => "Nosso app n&atilde;o executa o audio quando a tela esta desligada em alguns modelos de celular.<br><br>Para escutar nossa webtv seu celular deve estar com a tela ativa."),
	"en" => array("redes_sociais" => "Social Networks", "avaliar" => "Rate", "politica" => "Privacy Policy.", "compartilhar" => "Share", "pedir_musica" => "Request Song", "voltar" => "Back", "aviso_bg" => "Our app does not play audio when the screen is off on some cell phone models.<br><br>To listen to our webtv, your cell phone must have the screen active."),
	"es" => array("redes_sociais" => "Redes Sociales", "avaliar" => "Calificar", "politica" => "Pol&iacute;tica de Privacidad", "compartilhar" => "Compartir", "pedir_musica" => "Pedir Canci&oacute;n", "voltar" => "Volver", "aviso_bg" => "Nuestra aplicaci&oacute;n no reproduce audio cuando la pantalla est&aacute; apagada en algunos modelos de tel&eacute;fonos celulares.<br><br>Para escuchar nuestra webtv, su tel&eacute;fono celular debe tener la pantalla activa.")
);

/*
$db_player_app = @file_get_contents("app/player-app.db");
$busca_ip_db = explode("-", $db_player_app);
if(!in_array($_SERVER['REMOTE_ADDR'], $busca_ip_db)) {
@file_put_contents("app/player-app.db", $_SERVER['REMOTE_ADDR']."-", FILE_APPEND);
$exibir_aviso = "sim";
}
*/
?>
...
<nav id="sidebar">
   ...
   <div class="sidebar-header">
      <h4><?php echo utf8_encode($dados_stm["app_nome"]); ?></h3>
   </div>
...
<?php
if(query_string('3') == "checar-chave") {

$webrtc_chave = query_string('4');

if($dados_stm["webrtc_chave"] != $webrtc_chave || empty($webrtc_chave)){
unset($_SESSION["webrtc_chave"]);
echo "reload";
exit();
}
...
<!DOCTYPE html>
  ...
  <title>Player</title>
<style>*{margin:0;}html,body{height:99%; background-color:#000000;overflow: hidden;}#play-video-container{z-index: 999;position: absolute;display:flex;justify-content:center;align-items:center;}#play-video-container, #player-video { height: 100vh; width: 100%;}.plyr{display: none;} .plyr__time, .plyr__progress, .plyr__control--overlaid{display: none!important;}</style>
</head>
<body>
...
<video id="player-video" playsinline controls autoplay></video>
...
function checar_chave(){
    $.ajax({
    url: "/webrtc-app/<?php echo $dados_stm["login"]; ?>/player/checar-chave/<?php echo $dados_stm["webrtc_chave"]; ?>",
    success:
      function(resposta){
        if(resposta == "reload") {
