...
if($_POST["text"]) {
  mysqli_query($conexao,"INSERT INTO chat (login, nome, hash, ip, msg, data) VALUES ('".$dados_stm["login"]."', '".$_SESSION["nome_usuario"]."', '".$_SESSION["hash_usuario"]."', '".$_SERVER["REMOTE_ADDR"]."', '".addslashes($_POST["text"])."', NOW())");
exit();
}
...
small small {
  ...
  <body>
    <div class="container">
      ...
      <div class="col-md-12" id="quadro_msgs" style="<?php if(!isset($_SESSION["hash_usuario"])) { echo "display: none;"; }?>">
        <div class="panel panel-default">
          <div class="panel-heading">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
              <li class="droptitle message" style="cursor: pointer; padding-left: 10px;margin-bottom: 0px;" onClick="window.location = '/app-multi-plataforma/<?php echo query_string('1');?>/<?php echo $dados_stm["login"];?>/sair'"><i class="fa fa-sign-out"></i> <?php echo $array_lang[$dados_stm["idioma_painel"]]["btn_sair"]; ?></li>
            </ul>
          </div>
          </div>
      ...
      <div class="col-md-12" id="quadro_login" style="<?php if(isset($_SESSION["hash_usuario"])) { echo "display: none;"; }?>">
        <div class="panel panel-default">
          <div class="panel-heading">
            <i class="fa fa-comments"></i> Chat
          </div>
...
jQuery(document).ready(function() {
  ...
  $( "#sendform" ).submit(function(e) {
    if($('#text').val() != "") {
      $.post("/app-multi-plataforma/<?php echo query_string('1');?>/<?php echo $dados_stm["login"];?>", {text: $('#text').val()});
      $('#text').val("");
  ...
  $("#clickClear").click(function(e) {
    $.post("/app-multi-plataforma/<?php echo query_string('1');?>/<?php echo $dados_stm["login"];?>/limpar");
...
function listChat() 
{
  $.get("/app-multi-plataforma/<?php echo query_string('1');?>/<?php echo $dados_stm["login"];?>/lista", function (data) {
    $("#msg").append(data);
    if(window.bottom) {
...
function listOnline()
{
  $("#online").load("/app-multi-plataforma/<?php echo query_string('1');?>/<?php echo $dados_stm["login"];?>/online");
