...
//////////////////////////////////////////////////////////////////
/*
$checar_bloqueio_ip = @mysqli_num_rows(@mysqli_query($conexao,"SELECT * FROM bloqueios_login where ip = '".$_SERVER['REMOTE_ADDR']."' AND tentativas >= 5"));
...
$valida_usuario = mysqli_num_rows(mysqli_query($conexao,"SELECT * FROM streamings WHERE login = '".anti_sql_injection($_POST["login"])."' AND senha = '".anti_sql_injection($_POST["senha"])."'"));
...
@mysqli_query($conexao,"Delete From bloqueios_login where codigo = '".$dados_stm["codigo"]."'");
...
logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","".$lang['lang_info_log_login_painel']."");
...
//////////////////////////////////////////////////////////////////
/*
$checar_stm_bloqueio = @mysqli_num_rows(@mysqli_query($conexao,"SELECT * FROM streamings WHERE login = '".anti_sql_injection($_POST["login"])."'"));

if($checar_stm_bloqueio > 0) {

$checar_ip = @mysqli_num_rows(@mysqli_query($conexao,"SELECT * FROM bloqueios_login where ip = '".$_SERVER['REMOTE_ADDR']."'"));

if($checar_ip == 0) {

$dados_stm = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM streamings WHERE login = '".anti_sql_injection($_POST["login"])."'"));

@mysqli_query($conexao,"INSERT INTO bloqueios_login (codigo_cliente,codigo_stm,data,ip,navegador,tentativas) VALUES ('".$dados_stm["codigo_cliente"]."','".$dados_stm["codigo"]."',NOW(),'".$_SERVER['REMOTE_ADDR']."','".formatar_navegador($_SERVER['HTTP_USER_AGENT'])."','1')");

} else {
@mysqli_query($conexao,"Update bloqueios_login set tentativas = tentativas+1 where ip = '".$_SERVER['REMOTE_ADDR']."'");
}
